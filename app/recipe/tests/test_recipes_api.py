...
class PrivateTagsApiTests(TestCase):
    ...
    def test_create_recipe_with_ingredients(self):
        ...
        ingredient1 = sample_ingredient(user=self.user, name='Gouda')
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_to_recipe(self):
        """Test uplopading an image to recipe"""
    ...
    def test_upload_bad_image_to_recipe(self):
        """Test uplopading invalid image to recipe"""
        url = image_upload_url(self.recipe.id)
        res = self.client.post(url, {'image': 'justng'}, format='multipart')

        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_filter_recipe_by_tags(self):
        """Test returning recipe by tags"""
        recipe1 = sample_recipe(user=self.user, title='Thai Green Curry')
        recipe2 = sample_recipe(user=self.user, title='Penne Alio Oglio')
        tag1 = sample_tag(user=self.user, name='Spicy')
        tag2 = sample_tag(user=self.user, name='Simplistic')
    ...
    def test_filter_recipe_by_ingredients(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Thai Green Curry')
        recipe2 = sample_recipe(user=self.user, title='Penne Alio Oglio')
        ingredient1 = sample_ingredient(user=self.user, name='Roux')
